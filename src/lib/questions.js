...
export function getQuestions(desk, section) {
  const entry = Object.entries(files).find(([path]) => new RegExp(`/${desk}/\\d+-${section}/index\\.mdx$`).test(path));
  if (!entry) throw new Error(`QuestionIndex: no briefing section "${section}" in desk "${desk}"`);
