// Build-time only. Lists every risk written in each desk's briefing, so the Risks and Constraints risk map
...
export function getRisks() {
  ...
  for (const [path, text] of Object.entries(files)) {
    const [, desk, folder, file] = /briefings\/([^/]+)\/([^/]+)\/(\w+)\.mdx$/.exec(path);
    ...
    for (const r of found) {
      ...
      risks.push({ ...r, desk, section: slug, href: deskPath(desk, `/briefing/${slug}#${riskAnchor(r.title)}`) });
    }
  }
  return risks;
}
